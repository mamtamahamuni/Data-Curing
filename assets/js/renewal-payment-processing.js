...
var renewalPaymentProcessingApp = angular.module("renewalPaymentProcessingModule", []);

renewalPaymentProcessingApp.controller("renewal-payment-processing", ['ABHI_CONFIG', '$scope', '$rootScope', '$location', '$sessionStorage', 'RenewService', '$routeParams', '$timeout', function (ABHI_CONFIG, $scope, $rootScope, $location, $sessionStorage, RenewService, $routeParams, $timeout) {
    ...
    function userDropOff() {
        ...
        RenewService.postData(ABHI_CONFIG.apiUrl+"Renew/PageDrop", {
                    ...
                    "PageDrop": "renewal-payment-processing"
                },
                false
            )
            .then(function (data) {
    ...
    function omniDropOff(){
        ...
        RenewService.postData(ABHI_CONFIG.apiUrl+"OmniChannel/RenewDropoff", 
            {"ReferrenceNo": $sessionStorage.refNo, "PageName": "renewal-payment-processing"},
            true
            )
            .then(function(data){
            if(data.ResponseCode == 1) {
    ...
    function fetchPgStatus() {
        ...
        $sessionStorage.refNo = $scope.userpolicy.ReferenceNo;
        $sessionStorage.policyNo = $scope.userpolicy.PolicyNumber;
        $location.url('renewPaymentProcess');
        omniDropOff();
        }

        // RenewService.postData(ABHI_CONFIG.apiUrl+"Renew/PGStatus", {
    ...
    $rootScope.$on('$routeChangeSuccess', function (e, newLocation, oldLocation) {
        if (newLocation.$$route.originalPath == "/renewPaymentProcess" && oldLocation.$$route.originalPath == "/renewal-payment-processing") {
