...
app.directive('preQuote',['$timeout','$rootScope','$window',function ($timeout,$rootScope, $window) {
	return {
		scope:{
		...
		link: function (scope, element, attrs,$location,$anchorScroll){
			  ...
			  */
				...
				function switchSection(screenBox,nextSection,count,goingBack){
					...
					*/
					...
					/* End */
					if(count == 2 && scope.pQController.preQuote.ProductCategory == 'SC'){
						$('.ac-overlay').css('display','');
						
					}
					/*if(count == 3 && scope.pQController.preQuote.ProductCategory == 'SC'){
						
					}*/

					if(scope.pQController.previousSelectedCatogoryCode != scope.pQController.selectCategory && count == 2 && !goingBack){
						scope.pQController.selectedMembers = {};
						scope.pQController.coverTo = ""

						var lemniskObj = {
							"Protection against": scope.pQController.selectCategory
						};
					...
					if(screenBox.hasClass('screen-8')){
						$('.backToNext').show()
						scope.pQController.showBuyBtn = false
					}
					/* 
					...
					*/
						...
						if(count == 4){
							countSpan.text("Almost done");// Stored count value recived from argument of this function
						}else if(count == 3){
							$('.otherFamilyConstruct').removeClass('ul-activ');
							countSpan.text("Step 3");// Stored count value recived from argument of this function
						}else if(count == 2){
							countSpan.text("Step 2");// Stored count value recived from argument of this function
						}else{
							countSpan.text("Get Started "); // Stored count value recived from argument of this function
						}
						scope.pQController.count = count; // Stored count value recived from argument on scope

						if(scope.pQController.coverTo != "" && lemniskMemberCount == 0){
							var lemniskObj = {
								"Members": scope.pQController.coverTo
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
							lemniskMemberCount++;
						}
					...
					*/
						...
						if(location.hash == '#!/activ-secure-personal-accident-pre-quote-v2'){
							nextSection.removeClass('hide-on-load');
						}
			...
			*/
			...
			if(location.hash == '#!/activ-secure-personal-accident-pre-quote-v2'){
				$rootScope.callGtag('click-button','pre-quote','personal-accident-member-mobile');
				sessionStorage.removeItem('prequoteData');
				var oFC = $('.otherFamilyConstruct');
				// var inp = $('#ques_1_age');
				// inp.closest('.screen-box');	
						...
						*/
						...
						switch(inp.attr('id')){
							...
							case 'ques_4_13_age': // If id is 'ques_4_13_age' that means we are changing age of son
								...
								familyMemberChange($('#ques_4_13_age'),'soAge','son',18,45,16);
							break;
							...
							case 'ques_4_15_age': // If id is 'ques_4_15_age' that means we are changing age of daughter
								...
								familyMemberChange($('#ques_4_15_age'),'dulAge','daughter',18,45,18);
							break;
			...
			*/
				...
				$('.close-other-family-construct').on('click',function(){
					...
					*/
					if(scope.pQController.preQuote.ProductCategory == 'SC'){
						scope.pQController.selectedMembers = {}
				...
				scope.pQController.familyConstructSubmit = function(selectedMember){
					...
					}else if(location.href.includes('ABC_Xsell_1cr') && scope.pQController.productName == "Super-topup" && selectedMember.Self == false ){
						$rootScope.alertConfiguration('E',"Individual Family Construct exists only for Self. Please change your family construct." , "select-member_alert");
					}
					else{
			...
			*/
				...
				scope.pQController.submitActiveCareFamilycontruct = function(selectedMember){
					...
					if(angular.isUndefined(selectedMember)){
						...
						if(j == 0){
							...
							$rootScope.callGtag('click-button','pre-quote','prequote_AC-family-construct-submit');
							$('.otherFamilyConstruct').hide().removeClass('ul-activ'); // Hiding other family construct dropdown
							$('.ac-overlay').fadeOut();
							$('.q-input').removeClass('showOpt-open'); // Removing showOpt-open class
			...
			var tempMemberArr = []
			/* Change family construct on the click incase of Activ care */
				scope.pQController.chooseActiveCareMember = function(memberVal, selectmemberObj , membersObj){
					
					console.log(memberVal)
					console.log(selectmemberObj)
					console.log(membersObj)
					
							if(membersObj.RelationWithProposer  == 'Self' || membersObj.RelationWithProposer == 'Spouse'){
								unSelectMembers('Self' , 'Spouse')
							}	
							else if(membersObj.RelationWithProposer  == 'Father' || membersObj.RelationWithProposer == 'Mother'){
								unSelectMembers('Father' , 'Mother')
							}
							else if(membersObj.RelationWithProposer  == 'Father-in-law' || membersObj.RelationWithProposer == 'Mother-in-law'){
								unSelectMembers('Father-in-law' , 'Mother-in-law')
							}
							else if(membersObj.RelationWithProposer  == 'Brother' || membersObj.RelationWithProposer == 'Sister-In-Law'){
								unSelectMembers('Brother' , 'Sister-In-Law')
							}
							else if(membersObj.RelationWithProposer  == 'Sister' || membersObj.RelationWithProposer == 'Brother-In-Law'){
								unSelectMembers('Sister' , 'Brother-In-Law')
							}
							else if(membersObj.RelationWithProposer  == 'Grandfather' || membersObj.RelationWithProposer == 'Grandmother'){
								unSelectMembers('Grandfather' , 'Grandmother')
							}
							else if(membersObj.RelationWithProposer  == 'Uncle' || membersObj.RelationWithProposer == 'Aunt'){
								unSelectMembers('Uncle' , 'Aunt')
							}
				...
				function unSelectMembers( first ,second){
					angular.forEach(scope.pQController.selectedMembers , function ( i , j ){
									if(j != first && j != second ){
										scope.pQController.selectedMembers[j] = false;
									}
								})
				}

			/* Change family construct on the click incase of Activ care Ends*/
			...
			*/
				...
				function selfAgeChange($this){
					...
					if(angular.isUndefined(scope.pQController.preQuote.Age) || scope.pQController.preQuote.Age == "" || isNaN(scope.pQController.preQuote.Age)){ // If selft age isn't selected then we are showing alert to select age
						...
						switch(scope.pQController.preQuote.Cover) {
							case 'S': // If 'S' then we are directly activating next section as mobile section
								   var screenBox = $this.closest('.screen-box');
								   scope.pQController.selectedMembers = null
								   scope.pQController.Kids = null ;
								var nextSection = $(".screen-box[data-section='question-7']");
								   switchSection(screenBox,nextSection,4);
								break;
					...
					var lemniskObj = {
						"Self Age": scope.pQController.preQuote.Age
					};
					$rootScope.lemniskTrack("", "", lemniskObj);
				}
			...
			*/
				...
				function spuseAgeChange($this){
					...
					var lemniskObj = {
						"Spouse Age": scope.pQController.preQuote.spouseAge
					};
					$rootScope.lemniskTrack("", "", lemniskObj);
				}
			...
			*/
				...
				function familyMemberChange($this,member,memberText,minAge,maxAge,step){
					if(scope.pQController.preQuote.ProductCategory == 'SC' && scope.pQController.selectedMembers.Self){
						...
						if(memberText == "father"){
							var lemniskObj = {
								"Father Age": $this[0].value
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
						} else if(memberText == "mother"){
							var lemniskObj = {
								"Mother Age": $this[0].value
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
						} else if(memberText == "father-in-law"){
							var lemniskObj = {
								"Father-in-law Age": $this[0].value
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
						} else if(memberText == "mother-in-law"){
							var lemniskObj = {
								"Mother-in-law Age": $this[0].value
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
						}	
					}
					if(angular.isUndefined(scope.pQController.preQuote[member]) || scope.pQController.preQuote[member] == "" || isNaN(scope.pQController.preQuote[member])){
					...
					}else if((scope.pQController.preQuote[member] <= scope.pQController.preQuote.Age) && scope.pQController.preQuote.Cover != "SC" &&(memberText != "son" && memberText != "daughter")){
						$rootScope.alertConfiguration('E',"Age of "+memberText+" should not be lesser than self.",memberText+"_correct-age_alert");
					}else if(scope.pQController.preQuote.Cover == "D" && (memberText == "son" || memberText == "daughter") && (scope.pQController.preQuote[member] >= scope.pQController.preQuote.Age)){
						$rootScope.alertConfiguration('E',"Age of self should not be lesser than or same as " +memberText+"." ,memberText+"_correct-age_alert");
					}
					else{
						...
						if(memberText == "father"){
							...
							$rootScope.lemniskTrack("", "", lemniskObj);
						} else if(memberText == "son"){
							var lemniskObj = {
								"son Age": $this[0].value
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
						}
					}
				...
				function selectNunberOfKids($this){
					if(scope.pQController.preQuote.Kids > 4){
						...
						/*if(angular.isUndefined(scope.pQController.kidsObject) || scope.pQController.kidsObject.length != scope.pQController.preQuote.Kids){*/
							...
							var lemniskObj = {
								"Number of Kids": scope.pQController.kidsObject.length
							};
							$rootScope.lemniskTrack("", "", lemniskObj);
				...
				function lastChildAgeChange($this){
					...
					angular.forEach(scope.pQController.kidsObject,function(v,i){
						...
						if(v.age == "" || v.age == null || angular.isUndefined(v.age)){
							...
							else if(supText == "second"){
								var lemniskObj = {
									"Second Kid age" : v.age
								};
								$rootScope.lemniskTrack("", "", lemniskObj);
							}
							else if(supText == "third"){
								var lemniskObj = {
									"Third Kid age" : v.age
								};
								$rootScope.lemniskTrack("", "", lemniskObj);
							}
							else if(supText == "forth"){
								var lemniskObj = {
									"Forth Kid age" : v.age
								};
								$rootScope.lemniskTrack("", "", lemniskObj);
			...
			/* Mobile number change functions */
				...
				function mobileChange($this,event){
					...
					if($rootScope.ValidateBlockNo(mobileNumber)){
							mobileErrorMessage = "Please enter Valid Mobile No";
							
						}
					if(location.hash == '#!/activ-secure-personal-accident-pre-quote-v2'){
						scope.pQController.getMemder(mobileNumber,event);
						return;
					}
						...
						if(mobileErrorMessage == ""){
							...
							$('.backToNext').hide()
							scope.pQController.showBuyBtn = true
							   switchSection(screenBox,nextSection,4);
						}else{
